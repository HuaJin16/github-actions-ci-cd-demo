...
- `demo/start-clean` - clean passing state
- `demo/failing-pass-rate` - branch with intentional bug commit prepared
- `demo/fix-backup` - optional backup fix commit prepared

## Prepare issue and failing branch

1. Create a GitHub issue students will close (for example "Fix pass-rate regression").
2. Create and push a failing branch from `main`:

```bash
git checkout main
git pull origin main
git checkout -b demo/failing-pass-rate
# edit src/metrics.js to use (total / passed)
git add src/metrics.js
git commit -m "chore: introduce pass-rate regression for CI workshop"
git push -u origin demo/failing-pass-rate
```

3. Open PR from `demo/failing-pass-rate` to `main` and link the issue in the PR body.
4. Confirm CI fails on that PR.
...
4. Open the PR from `demo/failing-pass-rate` with the bug.
...
- Students fork repo and open PRs targeting `demo/failing-pass-rate`.
- Students only need CI checks for their learning loop.
- Instructor merges one selected student PR into `demo/failing-pass-rate`.
- Instructor then merges `demo/failing-pass-rate` into `main` to trigger CD.
